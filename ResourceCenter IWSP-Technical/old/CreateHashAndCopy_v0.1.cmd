...
REM echo ; CurrentPath: 		*%cd%>> _Checksums_%hash%.md5.txt
REM echo ; StartTime:   		*%date% %time%
REM echo Wait a minute to see time difference
REM pause
REM echo ; EndTime:     		*%date% %time% >> test-output.txt

REM date /T >> test-output.txt
REM time /T >> test-output.txt
REM goto End
...
REM create checksums and save them in file: _Checksums_%hash%.md5.txt
REM fsum -%hash% -r *.* >> _Checksums_%hash%.md5.txt
